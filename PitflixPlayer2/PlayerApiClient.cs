...
internal static class PlayerApiClient
{
    ...
    // ── Raw mpv commands (replaces direct IPC pipe) ───────────────────────────
    //
    // Routes any mpv command to the API, which forwards to libmpv or the named
    // pipe depending on the backend's UseLibMpv flag.  All argument types are
    // converted to strings: bool → "yes"/"no", numbers via InvariantCulture.

    public static Task SendMpvAsync(params object?[] args)
    {
        ...
        return SendMpvInternalAsync(strArgs);
    }

    private static async Task SendMpvInternalAsync(string[] args)
    {
        var body    = JsonSerializer.Serialize(new { args }, JsonOpts);
        var content = new StringContent(body, Encoding.UTF8, "application/json");
        try { await Http.PostAsync($"{BaseUrl}/mpv-command", content).ConfigureAwait(false); }
        catch { /* best-effort */ }
    }

    // ── Batch playlist build ──────────────────────────────────────────────────

    public static async Task SetPlaylistAsync(IReadOnlyList<string> files, string current,
                                              CancellationToken ct = default)
    {
        var body    = JsonSerializer.Serialize(new { files, current }, JsonOpts);
        var content = new StringContent(body, Encoding.UTF8, "application/json");
        try { await Http.PostAsync($"{BaseUrl}/playlist", content, ct).ConfigureAwait(false); }
        catch { /* best-effort */ }
    }
