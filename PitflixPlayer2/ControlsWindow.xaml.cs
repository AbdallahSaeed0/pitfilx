...
public partial class ControlsWindow : Window
{
    ...
    private void BuildPlaylistPopup()
    {
        ...
        for (int i = 0; i < _playlistFiles.Count; i++)
        {
            ...
            row.MouseLeftButtonDown += async (_, e) =>
            {
                ...
                App.Log($"Playlist: jumping to index {capturedIdx}");
                ...
                // mpv in a deterministic order.  Order: clear `start` (so the
                // new file begins at 0) → jump → unpause.
                await PlayerApiClient.SendMpvAsync("set_property", "start", "none").ConfigureAwait(true);
                await PlayerApiClient.SendMpvAsync("playlist-play-index", capturedIdx).ConfigureAwait(true);
                await PlayerApiClient.SendMpvAsync("set_property", "pause", false).ConfigureAwait(true);
            };
    ...
    private void OnPauseResumeClick(object sender, RoutedEventArgs e)
    {
        ...
        _ = PlayerApiClient.SendMpvAsync("set_property", "pause", _isPaused);
    }

    private async void OnPrevClick(object sender, RoutedEventArgs e)
    {
        SuspendThumbServer();        // free the disk for the file load
        // Clear `start` so this episode begins at 00:00, not the current file's resume offset.
        await PlayerApiClient.SendMpvAsync("set_property", "start", "none").ConfigureAwait(true);
        await PlayerApiClient.SendMpvAsync("playlist-prev").ConfigureAwait(true);
        await PlayerApiClient.SendMpvAsync("set_property", "pause", false).ConfigureAwait(true);
    }
    ...
    private async void OnNextClick(object sender, RoutedEventArgs e)
    {
        SuspendThumbServer();        // free the disk for the file load
        await PlayerApiClient.SendMpvAsync("set_property", "start", "none").ConfigureAwait(true);
        await PlayerApiClient.SendMpvAsync("playlist-next").ConfigureAwait(true);
        await PlayerApiClient.SendMpvAsync("set_property", "pause", false).ConfigureAwait(true);
    }
    ...
    private void OnSeekMouseUp(object sender, MouseButtonEventArgs e)
    {
        ...
        if (_duration > 0)
        {
            ...
            _ = PlayerApiClient.SendMpvAsync("seek", _seekTarget, "absolute");
        }
        e.Handled = true;
    }
    ...
    private void OnVolumeChanged(object sender, RoutedPropertyChangedEventArgs<double> e)
    {
        ...
        if (!_isVolumeDragging)
            _ = PlayerApiClient.SendMpvAsync("set_property", "volume", e.NewValue);
    }
    ...
    private void OnVolumeMouseDown(object sender, MouseButtonEventArgs e)
    {
        ...
        _ = PlayerApiClient.SendMpvAsync("set_property", "volume", VolumeSlider.Value);
        e.Handled = true;
    }
    ...
    private void OnSubtitleTrackClick(object sender, RoutedEventArgs e)
    {
        ...
        _ = PlayerApiClient.SendMpvAsync("set_property", "sid", idx == 0 ? "no" : idx.ToString());
        ...
        else
        {
            ...
            if (picked is not null && !string.IsNullOrWhiteSpace(picked.Language))
            {
                ...
                _ = PlayerApiClient.SendMpvAsync("set_property", "slang", picked.Language);
            }
            else
            {
    ...
    public async Task ApplyPreferredSubtitleAsync()
    {
        ...
        if (mode == "off")
        {
            await PlayerApiClient.SendMpvAsync("set_property", "sid", "no").ConfigureAwait(false);
        ...
        if (mode == "lang" && !string.IsNullOrWhiteSpace(App.Settings.PreferredSubLang))
        {
            ...
            // list and we'd pick the wrong (or non-existent) sid.
            ...
            {
                await PlayerApiClient.SendMpvAsync("set_property", "sid", match.Index).ConfigureAwait(false);
    ...
    private void OnMuteClick(object sender, RoutedEventArgs e)
    {
        if (_isMuted)
        {
            ...
            _ = PlayerApiClient.SendMpvAsync("set_property", "mute", false);
        ...
        else
        {
            ...
            _isMuted        = true;
            MuteIcon.Text   = "🔇";
            _ = PlayerApiClient.SendMpvAsync("set_property", "mute", true);
        }
    }
    ...
    private void ShowSpeedMenu(double ox, double oy)
    {
        ...
        foreach (var speed in SpeedOptions)
        {
            ...
            row.MouseLeftButtonUp += (_, _) =>
            {
                ...
                UpdateSpeedLabels();
                _ = PlayerApiClient.SendMpvAsync("set_property", "speed", captured);
            };
    ...
    private void OnAudioTrackClick(object sender, RoutedEventArgs e)
    {
        ...
        _ = PlayerApiClient.SendMpvAsync("set_property", "aid", idx);
    }
