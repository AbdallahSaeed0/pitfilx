...
use windows::Win32::{
  ...
  UI::WindowsAndMessaging::{
    ...
    GWL_EXSTYLE, HTTRANSPARENT, HWND_BOTTOM, SWP_NOACTIVATE, SWP_SHOWWINDOW, WM_NCHITTEST,
    WS_EX_TRANSPARENT,
  },
};
      ...
      loop {
        ...
        while let Ok(cmd) = cmd_rx.try_recv() {
          match cmd {
            ...
            PlayerCommand::SeekRelative(s) => {
              let _ = mpv_ev.command(&["seek", &format!("{s}"), "relative"]);
            }
            PlayerCommand::SeekAbsolute(s) => {
              let _ = mpv_ev.command(&["seek", &format!("{s}"), "absolute"]);
            }
...
pub fn player2_libmpv_set_bounds(
  ...
  app
    .run_on_main_thread(move || {
      ...
      // Convert CSS px (the JS side currently sends logical px) — keep 1:1; mpv child
      // is in pixel coords under the parent client area.
